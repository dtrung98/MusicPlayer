...
public class MusicService extends Service implements
  ...
  public void onCreate() {
    ...
    // queue saving needs to run on a separate thread so that it doesn't block the playback
    // handler events
          ...
          case ACTION_PLAY_PLAYLIST:
            ...
            if (playlist != null) {
              ...
              if (playlist instanceof AbsCustomPlaylist) {
                ...
                playlistSongs = (ArrayList<Song>) (List) PlaylistSongLoader.getPlaylistSongList(
                   getApplicationContext(),
                   playlist.id);
              }
  ...
  private synchronized void restoreQueuesAndPositionIfNecessary() {
    if (!queuesRestored && playingQueue.isEmpty()) {
      ArrayList<Song> restoredQueue = MusicPlaybackQueueStore.getInstance(this)
         .getSavedPlayingQueue();
      ArrayList<Song> restoredOriginalQueue = MusicPlaybackQueueStore.getInstance(this)
         .getSavedOriginalPlayingQueue();
      int restoredPosition = PreferenceManager.getDefaultSharedPreferences(this)
  ...
  public void initNotification() {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N &&
       !PreferenceUtil.getInstance(this).classicNotification()) {
  ...
  private void updateMediaSessionMetaData() {
    ...
    if (PreferenceUtil.getInstance(this).albumArtOnLockscreen()) {
      ...
      final RequestBuilder<Bitmap> request =
         SongGlideRequest.Builder.from(Glide.with(MusicService.this),
            song)
  ...
  public void openQueue(
    ...
    if (playingQueue != null && !playingQueue.isEmpty() && startPosition >= 0 &&
      ...
      if (requestFocus()) {
        if (!playback.isPlaying()) {
          if (!playback.isInitialized()) {
            ...
            // fixes a bug where the volume would stay ducked because the AudioManager
            // .AUDIOFOCUS_GAIN event is not sent
        ...
        Toast.makeText(this,
           getResources().getString(R.string.audio_focus_denied),
  ...
  public void updateVolume() {
    try {
      playback.setVolume(mCurrentVolume * mInAppVolume * mLeftBalanceValue,
         mCurrentVolume * mInAppVolume * mRightBalanceValue);
