...
public class SongLoader {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && !forceOldWay) {
      ...
      try {
        Files.walk(Paths.get(root))
           .filter(path -> NO_MEDIA_TAG.equals(path.getFileName().toString()))
           .map(Path::toFile)
           .collect(Collectors.toCollection(() -> noMedias));
      } catch (Exception ignored) {
      }
      for (int i = 0, noMediaPathsSize = noMedias.size(); i < noMediaPathsSize; i++) {

        try {
          Files.walk(Paths.get(noMedias.get(i).getParent()))
             .filter(path -> Files.isRegularFile(path) && isAudioExtension(path))
             .map(Path::toFile)
  ...
  private static boolean isAudioExtension(Path path) {
    ...
    String mime = MimeTypeMap.getSingleton()
       .getMimeTypeFromExtension(getFileExtension(path.toString()));
  ...
  public static long[] doFindAudioWayTwo(@NonNull Context context) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      try {
        Files.walk(Paths.get(root))
           .filter(path -> NO_MEDIA_TAG.equals(path.getFileName().toString()))
           .collect(Collectors.toCollection(() -> noMediaPaths));
      } catch (Exception e) {
        e.printStackTrace();
      }
      ...
      try {
        Files.walk(Paths.get(root))
           .filter(path -> !Files.isDirectory(path) && noMediaPaths.contains(path.getParent()) &&
              isAudioExtension(path))
           .collect(Collectors.toCollection(() -> audioPaths));
      } catch (Exception ignored) {
      }
    }
    long end = System.nanoTime();
  ...
  public static void doSomething(@NonNull Context context) {
   ...
   /*     w1[1] = 0;
    ...
    if (true) return;
    String where = MediaStore.Files.FileColumns.MIME_TYPE + " = 'audio/mpeg'" +
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      try {
        //Files.walk(Paths.get(root.getAbsolutePath())).filter(path -> Files.isRegularFile(path)
        // &&"audio/mpeg".equals(MimeTypeMap.getSingleton().getMimeTypeFromExtension
        // (getFileExtension(path)))).forEach(x -> Log.d(TAG, "find file "+x.toAbsolutePath()));
        Files.walk(Paths.get(root.getAbsolutePath()))
           .filter(path -> NO_MEDIA_TAG.equals(path.getFileName().toString()))
           .forEach(x -> Log.d(TAG, "find file " + x.toAbsolutePath()));
      } catch (Exception ignored) {
      }
    }
    if (true) return;
          ...
          if (file.exists()) {
            ...
            if (audioFile != null) {
              ...
              if (tag != null) {
                ...
                Log.d(TAG,
                   "find hidden song: title = " + title + ", artist = " + artist + ", album = " +
                      album + ", genre = " + genre);
              }
            }
          }
  ...
  @Nullable
  public static Cursor makeSongCursor(
  ...
  private static String addMinDurationFilter(String selection) {
    return selection + " AND " + AudioColumns.DURATION + " > " +
       App.getInstance().getPreferencesUtility().getMinDuration();
  }
  ...
  private static String[] addBlacklistSelectionValues(
     String[] selectionValues,
