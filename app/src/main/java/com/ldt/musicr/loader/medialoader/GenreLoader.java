...
public class GenreLoader {
  ...
  @NonNull
  private static ArrayList<Genre> getGenresFromCursor(
          ...
          if (genre.songCount > 0) {
            ...
            try {
              context.getContentResolver()
                 .delete(Genres.EXTERNAL_CONTENT_URI, Genres._ID + " == " + genre.id, null);
            } catch (Exception e) {
              e.printStackTrace();
  ...
  @NonNull
  private static Genre getGenreFromCursor(
     @NonNull final Context context,
  ...
  public static Cursor getGenreCursorForArtist(final Context context, long artistId) {
    String query =
       " _id in (select genre_id from audio_genres_map where audio_id in (select _id from " +
          "audio_meta where " +
          SongLoader.BASE_SELECTION + " AND artist_id = " + artistId + "))";
    final String[] projection = new String[]{
       Genres._ID,
       Genres.NAME
    };
    try {
      return context.getContentResolver().query(
         ...
         query,
         null,
         SortOrder.GenreSortOrder.GENRE_A_Z/*PreferenceUtil.getInstance(context)
         .getGenreSortOrder()*/);
    } catch (Exception e) {
      e.printStackTrace();
      return null;
