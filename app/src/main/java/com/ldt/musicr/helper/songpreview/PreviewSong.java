...
public class PreviewSong {
  ...
  private static final float DELTA_IN_VOLUME =
     (DEFAULT_PLAY_VOLUME - DEFAULT_IN_VOLUME) / (float) NUMBER_OF_STEPS_IN;
  private static final int NUMBER_OF_STEPS_OUT = FADE_OUT_DURATION / FADE_INTERVAL;
  private static final float DELTA_OUT_VOLUME =
     (DEFAULT_PLAY_VOLUME - DEFAULT_OUT_VOLUME) / (float) NUMBER_OF_STEPS_OUT;
  ...
  public void play() {
    ...
    try {
      ...
      mMediaPlayer.setOnPreparedListener(mp -> {
        ...
        Log.d(TAG,
           "play: init = " + (time1 - start) + ", setDataSource = " + (time2 - time1) +
              ", prepare = " + (System.currentTimeMillis() - time2));
  ...
  private void onAudioStartFadeIn() {
    ...
    setState(PLAYING);
    Log.d(TAG,
       "media start time = " + (time1 - start) + ", notify time = " +
  ...
  private void updateVolume(final int why) {
    ...
    try {
      mMediaPlayer.setVolume(mCurrentVolume * getInAppVolume() * getLeftBalanceValue(),
         mCurrentVolume * getInAppVolume() * getRightBalanceValue());
      if (startFadeOut != -1) {
        Log.d(TAG,
           why + " : song " + mSong.title + ", update new volume = " + mCurrentVolume +
              ", thread id " + Thread.currentThread().getId() + ", time fade out = " +
              (System.currentTimeMillis() - startFadeOut));
      } else {
        Log.d(TAG,
           why + " : song " + mSong.title + ", update new volume = " + mCurrentVolume +
  ...
  private void fadeIn() {
    ...
    mPlayHandler.postDelayed(mFinishPlayRunnable,
       mFinish - mStart - FADE_OUT_DURATION - FADE_IN_DURATION);
    mFadeState = FADE_IN;
    update();
  }
