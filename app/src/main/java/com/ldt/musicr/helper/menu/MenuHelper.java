...
public class MenuHelper {
  ...
  public static boolean handleMenuClick(
    ...
    switch (string_res_option) {
      ...
      case R.string.add_to_playlist:
        AddToPlaylistDialog.create(new ArrayList<>(artist.getSongs()))
  ...
  public static boolean handleMenuClick(
    ...
    switch (string_res_option) {
      case R.string.play_next:
        MusicPlayerRemote.playNext(new ArrayList<>(MusicUtil.getPlaylistSongList(activity,
           playlist,
           "")));
        return true;
      case R.string.play_preview:
        if (activity instanceof AppActivity) {
          ...
          if (preview != null) {
            if (preview.isPlayingPreview()) {
              ...
              ArrayList<Song> list = new ArrayList<>(MusicUtil.getPlaylistSongList(activity,
                 playlist,
                 ""));
              Collections.shuffle(list);
              preview.previewSongs(list);
        ...
        return true;
      case R.string.add_to_queue:
        MusicPlayerRemote.enqueue(new ArrayList<>(MusicUtil.getPlaylistSongList(activity,
           playlist,
           "")));
        return true;
      case R.string.add_playlist_to_playlist:
        AddToPlaylistDialog.create(new ArrayList<>(MusicUtil.getPlaylistSongList(activity,
           playlist,
           ""))).show(activity.getSupportFragmentManager(), "ADD_PLAYLIST");
        return true;
  ...
  private static class SavePlaylistAsyncTask extends
     WeakContextAsyncTask<Playlist, String, String> {
    ...
    protected String doInBackground(Playlist... params) {
      try {
        return String.format(App.getInstance()
              .getApplicationContext()
              .getString(R.string.saved_playlist_to),
           PlaylistsUtil.savePlaylist(App.getInstance().getApplicationContext(), params[0]));
      } catch (Exception e) {
        e.printStackTrace();
        return String.format(App.getInstance()
           .getApplicationContext()
           .getString(R.string.failed_to_save_playlist), e);
      }
    }
