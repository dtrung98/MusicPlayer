...
public class SongPreviewController implements MusicServiceEventListener, SongPreviewListener {
  ...
  private static class SoundFilesLoader extends AsyncTask<List<Song>, Void, Void> {
    ...
    private void loadPreviewSong(Song song) {
      ...
      Log.d(TAG,
         ("loadPreviewSong costs " + (end - start) + "ms: creating sound file costs " +
            (middle - start) + ", calculating costs " + (end - middle)));
    ...
    private synchronized void calculateSound(SoundFile soundFile) {
      ...
      int numberPenGain_Guess = (int) (Math.floor(
         ((frameToZone - frameFromZone) / (mNumFrames + 0f)) *
            (mDuration / penGainSecond_Guessing))); // each 500ms
      int numberFrameEachPenGain = numberFrameGainPlayingZone / numberPenGain_Guess;

      numberPenGain_Guess = (int) Math.ceil(
         (numberFrameGainPlayingZone + 0f) / numberFrameEachPenGain);
      ...
      for (int iFrame = 0; iFrame < numberFrameGainPlayingZone; iFrame++) {
        ...
        if (iPen == numberPenGain_Guess - 1) {
          Log.d(TAG,
             "last : frame[" + iFrame + "] = " + frameGainInPlayingZone[iFrame] + ", pos = " + pos);
        }

        if (iFrame == numberFrameGainPlayingZone - 1) {
          originalPenGain[iPen] /= pos;
          Log.d(TAG,
             "last : ipen = " + iPen + ", average = " + originalPenGain[iPen] + ", from " + pos +
                " element" + " / " + numberFrameEachPenGain);
      ...
      //            SmoothedPenGain[i_pen] = getHeight(i_pen, numberPenGain_Guess,
      //            originalPenGain, scaleFactor, minGain, range);
      ...
      Log.d(TAG,
         "calculateSound: start = " + newStartPen + ", penNum = " + (newEndPen - newStartPen));
