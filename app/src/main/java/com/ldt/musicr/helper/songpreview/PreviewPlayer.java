...
public class PreviewPlayer implements PreviewSong.OnPreviewSongStateChangedListener,
  ...
  public void play(boolean stillPlayIfPaused) {
    ...
    if (!mPreviewSongs.isEmpty() && !mIsUserPaused) {
      ...
      Log.d(TAG,
         "onPreviewSongStateChanged : isEmpty = " + mPreviewSongs.isEmpty() + ", shouldPlayMS = " +
            mShouldPlayingMusicServiceOnFinish + ", MPR is playing = " +
            MusicPlayerRemote.isPlaying());
      if (mPreviewSongs.isEmpty() && mShouldPlayingMusicServiceOnFinish &&
         !MusicPlayerRemote.isPlaying()) {
  ...
  public void onPreviewSongStateChanged(PreviewSong song, int newState) {
    ...
    if (!mPreviewSongs.isEmpty() && mPreviewSongs.get(0).equals(song) &&
       (newState == PreviewSong.PREPARE_TO_FINISH || newState == PreviewSong.FAILURE)) {
      song.removeListener();
      ...
      mListener.onSongPreviewFinish(song);
        ...
        /*else if(newState == PreviewSong.PREPARE_TO_FINISH|| newState == PreviewSong.FAILURE) {
            Log.d(TAG, "onPreviewSongStateChanged : isEmpty = " + mPreviewSongs.isEmpty() + ",
            shouldPlayMS = " + mShouldPlayingMusicServiceOnFinish + ", MPR is playing = " +
            MusicPlayerRemote.isPlaying());
            if (mPreviewSongs.isEmpty() && mShouldPlayingMusicServiceOnFinish &&
            !MusicPlayerRemote.isPlaying()) {
                MusicPlayerRemote.playOrPause();
