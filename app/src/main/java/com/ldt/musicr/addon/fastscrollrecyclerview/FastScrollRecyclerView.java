...
public class FastScrollRecyclerView extends RecyclerView implements
   ...
   * @param rowCount       the number of rows, used to calculate the total scroll height
   *                       (assumes that
   *                       all rows are the same height)
   * @param yOffset        the offset to start tracking in the recycler view (only used for all
   *                      apps)
  ...
  protected void updateThumbPosition(
     ScrollPositionState scrollPosState,
     int rowCount,
     int yOffset
  ) {
    ...
    // padding)
    int scrollY = getPaddingTop() + yOffset + (scrollPosState.rowIndex * scrollPosState.rowHeight) -
   ...
   *                       apps)
   */
  protected void updateThumbPositionWithMeasurableAdapter(
     ScrollPositionState scrollPosState,
     int yOffset
  ) {
  ...
  private void getCurScrollState(ScrollPositionState stateOut) {
    ...
    if (getLayoutManager() instanceof GridLayoutManager) {
      stateOut.rowIndex =
         stateOut.rowIndex / ((GridLayoutManager) getLayoutManager()).getSpanCount();
    }
  ...
  /**
   * Set the FastScroll Popup position. This is either
   * {@link FastScroller.FastScrollerPopupPosition#ADJACENT},
   * meaning the popup moves adjacent to the FastScroll thumb, or
   * {@link FastScroller.FastScrollerPopupPosition#CENTER},
