...
public class NavigationUtil {
  ...
  public static void navigateToBackStackController(@NonNull final AppActivity activity) {
    final CardLayerController.CardLayerAttribute playingQueueAttr =
       activity.getCardLayerController()
       .getMyAttr(activity.getPlayingQueueLayerFragment());
    final CardLayerController.CardLayerAttribute nowPlayingAttr = activity.getCardLayerController()
       .getMyAttr(activity.getNowPlayingController());

    if (playingQueueAttr.getState() != CardLayerController.CardLayerAttribute.MINIMIZED &&
       nowPlayingAttr.getState() != CardLayerController.CardLayerAttribute.MINIMIZED) {
  ...
  public static void navigateToArtist(@NonNull final Activity activity, final int artistId) {
    if (activity instanceof AppActivity) {
      ...
      if (fragment != null) {
        fragment.getNavigationController()
           .presentFragment(ViewArtistFragment.newInstance(MediaManager.INSTANCE.getArtist(artistId)));
      }
  ...
  public static void navigateToPlaylist(@NonNull final Activity activity, final Playlist playlist) {
    if (activity instanceof AppActivity) {
      ...
      if (fragment != null) {
        fragment.getNavigationController()
           .presentFragment(ViewPlaylistFragment.newInstance(playlist, null));
      }
      navigateToBackStackController(appActivity);
    }
  }
