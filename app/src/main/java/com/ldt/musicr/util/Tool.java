...
public class Tool {
  ...
  private Bitmap blurWallPaper;
    /*private Bitmap getCropCenterScreenBitmap(Bitmap source_bitmap) {
        ...
        if(ratio_parent> ratio_source) {
            ...
            rect_parent_in_bitmap.Height = (int) (rect_parent_in_bitmap
            .Width*parentHeight/parentWidth);

            rect_parent_in_bitmap.Left = 0;
            rect_parent_in_bitmap.Top = source_bitmap.getHeight()/2 - rect_parent_in_bitmap
            .Height/2;
        } else {
            // crop width of source
            // mean that
            rect_parent_in_bitmap.Height = source_bitmap.getHeight();
            rect_parent_in_bitmap.Width = (int) (rect_parent_in_bitmap
        ...
        Bitmap ret = Bitmap.createBitmap((int)parentWidth, (int) parentHeight,Bitmap.Config
        .ARGB_8888);
        Canvas canvas = new Canvas(ret);
        canvas.drawBitmap(source_bitmap,rect_parent_in_bitmap.getRectGraphic(),new Rect(0,0,
        screenSize[0],screenSize[1]),null);
        return ret;
    }*/
  ...
  public static int[] getScreenSize(Context context) {
    if (!HAD_GOT_SCREEN_SIZE) {
      ...
      // this will get the view of screen
      d.getRealSize(p);
      int width = p.x;
      int height = p.y;
      screenSize = new int[]{width, height};
      screenSizeInDp = new float[]{
         (width + 0.0f) / getOneDps(context),
         (height + 0.0f) / getOneDps(context)
      };
  ...
  public static String convertByteArrayToString(byte[] b) {
    ...
    for (int i = 0; i < len; i++) {
      if (i != len - 1) {
        r.append(Integer.toHexString(b[i])).append(":");
      } else {
        r.append(b[i]);
      }
    }
  ...
  public static void showToast(Context context, String text, int time) {
    ...
    toast = Toasty.custom(context,
       text,
       R.drawable.emoticon_excited,
       R.color.library_back_color,
       time,
       true,
       true);
  ...
  public static class Avatar {
    public static int getDevideSize(int sizeUWant, Bitmap original) {
      ...
      int sizeOriginal = (original_height <
         original_width) ? original_width : original_height; // lấy cái lớn hơn
