...
public class CustomArtistImageUtil {
  ...
  private CustomArtistImageUtil(@NonNull final Context context) {
    mPreferences = context.getApplicationContext()
       .getSharedPreferences(CUSTOM_ARTIST_IMAGE_PREFS, Context.MODE_PRIVATE);
  }
    ...
    Glide.with(App.getInstance())
       ...
       .into(new SimpleTarget<Bitmap>() {
         ...
         public void onLoadFailed(Drawable errorDrawable) {
           Toast.makeText(App.getInstance(), "Sorry, something went wrong :((", Toast.LENGTH_LONG)
              .show();
         }
           ...
           new AsyncTask<Void, Void, Void>() {
             ...
             protected Void doInBackground(Void... params) {
               ...
               if (succesful) {
                 ...
                 ArtistSignatureUtil.getInstance(App.getInstance())
                    .updateArtistSignature(artist.getName());
                 App.getInstance()
                    .getContentResolver()
                    .notifyChange(Uri.parse("content://media"),
