...
public class BlurTransformation extends BitmapTransformation {
  ...
  protected Bitmap transform(BitmapPool pool, Bitmap toTransform, int outWidth, int outHeight) {
    int sampling;
    if (this.sampling == 0) {
      sampling = ImageUtil.calculateInSampleSize(toTransform.getWidth(),
    ...
    if (Build.VERSION.SDK_INT >= 17) {
      try {
        ...
        // on some devices RenderScript.create() throws: android.support.v8.renderscript
        // .RSRuntimeException: Error loading libRSSupport library
  ...
  public static class Builder {
    ...
    /**
     * @param sampling The inSampleSize to use. Must be a power of 2, or 1 for no down
     *                 sampling or 0 for auto detect sampling. Default is 0.
     * @return the same Builder
     */
