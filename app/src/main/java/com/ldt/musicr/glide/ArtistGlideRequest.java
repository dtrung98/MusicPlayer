...
public class ArtistGlideRequest {
  ...
  public static RequestBuilder<Drawable> createBaseRequestForDrawable(
    ...
    boolean hasCustomImage = CustomArtistImageUtil.getInstance(App.getInstance())
       .hasCustomArtistImage(artist);
    if (noCustomImage || !hasCustomImage) {
      builder = requestManager.load(new ArtistImage(artist.getName(),
         forceDownload,
         loadOriginal,
         imageNumber));
    } else {
  ...
  public static RequestBuilder<Bitmap> createBaseRequest(
     RequestManager requestManager,
     Artist artist,
     boolean noCustomImage,
     boolean forceDownload,
    ...
    if (noCustomImage || !hasCustomImage) {
      builder = requestManager.asBitmap()
         .load(new ArtistImage(artist.getName(), forceDownload, loadOriginal, imageNumber));
    } else {
  ...
  public static Key createSignature(Artist artist, boolean isLoadOriginal, int whichImage) {
    return ArtistSignatureUtil.getInstance(App.getInstance())
       .getArtistSignature(artist.getName(), isLoadOriginal, whichImage);
  }
  ...
  public static class Builder {
    ...
    public RequestBuilder<Bitmap> build() {
      return createBaseRequest(requestManager,
         artist,
         noCustomImage,
         forceDownload,
         mLoadOriginalImage,
  ...
  public static class BitmapBuilder {
    ...
    public RequestBuilder<Bitmap> build() {
      ...
      return createBaseRequest(builder.requestManager,
         builder.artist,
         builder.noCustomImage,
         builder.forceDownload,
         builder.mLoadOriginalImage,
         builder.mImageNumber)
         .diskCacheStrategy(DEFAULT_DISK_CACHE_STRATEGY)
         .priority(Priority.LOW)
         .override(Target.SIZE_ORIGINAL, Target.SIZE_ORIGINAL)
    ...
    public RequestBuilder<Drawable> buildRequestDrawable() {
      //noinspection unchecked
      return createBaseRequestForDrawable(builder.requestManager,
         builder.artist,
         builder.noCustomImage,
         builder.forceDownload,
         builder.mLoadOriginalImage,
