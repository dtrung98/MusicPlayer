...
public class ArtistImageFetcher implements DataFetcher<InputStream> {
    ...
    public String getId() {
        ...
        return String.valueOf(model.mArtistName);
    }

    @Override
    public InputStream loadData(Priority priority) throws Exception {
        if (!MusicUtil.isArtistNameUnknown(model.mArtistName)/* && PreferenceUtil.isAllowedToDownloadMetadata(context)*/) {
            Response<LastFmArtist> response = lastFMRestClient.getApiService().getArtistInfo(model.mArtistName, null, model.mSkipOkHttpCache ? "no-cache" : null).execute();
