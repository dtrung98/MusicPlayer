...
public class ArtistImageFetcher implements DataFetcher<InputStream> {
  ...
  private Exception loadThisArtistWithJSoup(
     String artistName,
    ...
    try {
      response = lastFMRestClient.getApiService()
         .getArtistInfo(artistName, null, model.mSkipOkHttpCache ? "no-cache" : null)
         .execute();
    ...
    if (!response.isSuccessful()) {
      ...
      String largestArtistImageUrl = LastFMUtil.getLargestArtistImageUrl(lastFmArtist.getArtist()
         .getImage());
      if (largestArtistImageUrl != null && !largestArtistImageUrl.isEmpty()) {

         /*     Response<String> photoPage = null;
                try {
                    photoPage = mLastFmClient.getApiService().getPhotoPage(lastFmArtist.getArtist
                    ().getUrl()+"/+images",null,mSkipOkHttpCache ? "no-cache" : null).onRunning();
                } catch (IOException e) {
                    e.printStackTrace();
                    return e;
                }
        ...
        if (document != null) {
          ...
          if (imageList != null) {
            ...
            if (imageListItem != null) {
              ...
              switch (mImageNumber) {
                ...
                default:
                  if (mImageNumber > result.size() - 1) {
                    ...
                    urlString = result.get(mImageNumber);
                  }
              }
              ...
              ModelLoader.LoadData<InputStream> loadData = urlLoader.buildLoadData(url,
                 width,
                 height,
                 mOption);
              if (loadData == null) {
                ...
                try {
                  ...
                  DataCallback<? super InputStream> innerCallback =
                     new DataCallback<InputStream>() {
                       @Override
...
/*
                if(mLoadOriginal) largestArtistImageUrl = ArtistImageFetcher
                .findAndReplaceToGetOriginal(largestArtistImageUrl);
  ...
  public void loadData(
    ...
    if (!MusicUtil.isArtistNameUnknown(model.mArtistName)/* && PreferenceUtil
    .isAllowedToDownloadMetadata(context)*/) {
      ...
      if (null == loadThisArtistWithJSoup(artistNames.replace("&", ", ")
         .replaceAll("( +)", " ")
         .trim()
         .replaceAll("\\s+(?=[),])", ""), priority, callback)) {
        return;
      }
      ...
      callback.onLoadFailed(new Exception("Unknown Artist"));
    }
  }
