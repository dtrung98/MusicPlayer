...
open class NormalSongViewHolder(
  parent: ViewGroup,
  private val actionResponder: ActionResponder?,
  layoutInflater: LayoutInflater? = null
) : RecyclerView.ViewHolder(
  (layoutInflater ?: LayoutInflater.from(parent.context)).inflate(
    R.layout.item_song_normal,
    parent,
    false
  )
), BindPlayingState, BindPreviewState, BindTheme {
  ...
  init {
    ...
    itemView.findViewById<View>(R.id.menu_button).setOnClickListener {
      data?.also {
        ItemViewUtils.handleMenuClickOnNormalSongItem(
          this,
          it.song,
  ...
  fun bind(item: DataItem.SongItem, previewingSong: PreviewSong? = null) {
    ...
    numberView.text =
      numberView.resources.getString(R.string.str_number_placeholder, item.positionInData + 1)
  ...
  private fun bindPreviewButton(song: Song, previewingSong: PreviewSong?) {
    if ((previewingSong == null || previewingSong.song.id != song.id) && previewView.mode == CircularPlayPauseProgressBar.PLAYING) {
      ...
      if (timePlayed == -1L) previewView.resetProgress() else {
        ...
        if (timePlayed <= previewingSong.totalPreviewDuration) previewView.syncProgress(
          previewingSong.totalPreviewDuration,
          timePlayed.toInt()
        )
      }
    }
