...
public class RangeSeekBar extends View {
  ...
  private void initProgressLine() {
    ...
    if (rightSB == null) {
      lineTop = (int) (leftSB.getIndicatorHeight() + leftSB.getIndicatorArrowSize() +
         leftSB.getThumbSize() * leftSB.getThumbScaleRatio() / 2 - progressHeight / 2);
    } else {
      lineTop = (int) (Math.max(leftSB.getIndicatorHeight() + leftSB.getIndicatorArrowSize() +
            leftSB.getThumbSize() * leftSB.getThumbScaleRatio() / 2,
         rightSB.getIndicatorHeight() + rightSB.getIndicatorArrowSize() +
            rightSB.getThumbSize() / 2) - progressHeight / 2);
    }
  ...
  private void initAttrs(AttributeSet attrs) {
    ...
    progressDefaultColor = t.getColor(R.styleable.RangeSeekBar_rsb_progress_default_color,
       0xFFD7D7D7);
    progressHeight = (int) t.getDimension(R.styleable.RangeSeekBar_rsb_progress_height,
       Utils.dp2px(getContext(), 2));
    tickMarkMode = t.getInt(R.styleable.RangeSeekBar_rsb_tick_mark_mode, TRICK_MARK_MODE_NUMBER);
    tickMarkGravity = t.getInt(R.styleable.RangeSeekBar_rsb_tick_mark_gravity,
       TRICK_MARK_GRAVITY_CENTER);
    ...
    tickMarkTextMargin = (int) t.getDimension(R.styleable.RangeSeekBar_rsb_tick_mark_text_margin,
       Utils.dp2px(getContext(), 7));
    tickMarkTextSize = (int) t.getDimension(R.styleable.RangeSeekBar_rsb_tick_mark_text_size,
       Utils.dp2px(getContext(), 12));
    tickMarkTextColor = t.getColor(R.styleable.RangeSeekBar_rsb_tick_mark_text_color,
       progressDefaultColor);
    tickMarkInRangeTextColor = t.getColor(R.styleable.RangeSeekBar_rsb_tick_mark_text_color,
       progressColor);
    t.recycle();
  }
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (tickMarkTextArray != null) {
      ...
      for (int i = 0; i < tickMarkTextArray.length; i++) {
        ...
        if (tickMarkMode == TRICK_MARK_MODE_OTHER) {
          ...
          if (Utils.compareFloat(num, states[0].value) != -1 &&
             Utils.compareFloat(num, states[1].value) != 1 && seekBarMode == SEEKBAR_MODE_RANGE) {
    ...
    if (seekBarMode == SEEKBAR_MODE_RANGE) {
      ...
      foregroundLineRect.left =
         leftSB.left + leftSB.getThumbSize() / 2 + lineWidth * leftSB.currPercent;
      foregroundLineRect.right =
         rightSB.left + rightSB.getThumbSize() / 2 + lineWidth * rightSB.currPercent;
  ...
  public void setValue(float leftValue, float rightValue) {
    ...
    if (leftValue < minProgress) {
      throw new IllegalArgumentException(
         "setValue() min < (preset min - offsetValue) . #min:" + leftValue + " #preset min:" +
            rightValue);
    }
    if (rightValue > maxProgress) {
      throw new IllegalArgumentException(
         "setValue() max > (preset max - offsetValue) . #max:" + rightValue + " #preset max:" +
            rightValue);
    }
  ...
  public void setRange(float min, float max, float interval, int tickMarkNumber) {
    if (max <= min) {
      throw new IllegalArgumentException(
         "setRange() max must be greater than min ! #max:" + max + " #min:" + min);
    }
    if (interval < 0) {
      throw new IllegalArgumentException(
         "setRange() interval must be greater than zero ! #interval:" + interval);
    }
    if (interval >= max - min) {
      throw new IllegalArgumentException(
         "setRange() interval must be less than (max - min) ! #interval:" + interval +
            " #max - min:" + (max - min));
    }
    if (tickMarkNumber < 1) {
      throw new IllegalArgumentException(
         "setRange() tickMarkNumber must be greater than 1 ! #tickMarkNumber:" + tickMarkNumber);
    }
    maxProgress = max;
    minProgress = min;
    ...
    minRangeCells = (int) (reservePercent / cellsPercent +
       (reservePercent % cellsPercent != 0 ? 1 : 0));

    if (tickMarkNumber > 1) {
      if (rightSB != null) {
        if (leftSB.currPercent + cellsPercent * minRangeCells <= 1 &&
           leftSB.currPercent + cellsPercent * minRangeCells > rightSB.currPercent) {
          rightSB.currPercent = leftSB.currPercent + cellsPercent * minRangeCells;
        } else if (rightSB.currPercent - cellsPercent * minRangeCells >= 0 &&
           rightSB.currPercent - cellsPercent * minRangeCells < leftSB.currPercent) {
          leftSB.currPercent = rightSB.currPercent - cellsPercent * minRangeCells;
        }
      } else {
        if (1 - cellsPercent * minRangeCells >= 0 &&
           1 - cellsPercent * minRangeCells < leftSB.currPercent) {
  ...
  public SeekBarState[] getRangeSeekBarState() {
    ...
    if (tickMarkNumber > 1) {
      ...
      leftSeekBarState.indicatorText =
         (new StringBuffer().append(leftSeekBarState.value)).toString();
    ...
    if (rightSB != null) {
      ...
      if (tickMarkNumber > 1) {
        ...
        rightSeekBarState.indicatorText =
           (new StringBuffer().append(rightSeekBarState.value)).toString();
  ...
  private void scaleCurrentSeekBarThumb() {
    if (currTouchSB != null && currTouchSB.getThumbScaleRatio() > 1f && !isScaleThumb) {
      isScaleThumb = true;
      currTouchSB.setThumbSize((int) (currTouchSB.getThumbSize() *
         currTouchSB.getThumbScaleRatio()));
  ...
  public boolean onTouchEvent(MotionEvent event) {
    ...
    switch (event.getAction()) {
      case MotionEvent.ACTION_DOWN:
        ...
        if (rightSB != null && rightSB.currPercent >= 1 &&
           leftSB.collide(getEventX(event), getEventY(event))) {
          currTouchSB = leftSB;
          touchResult = true;
