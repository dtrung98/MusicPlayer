...
class VisualSeekBarParser implements ProgressListener {
   ...
   * Step 2: When the view size available, or change, use the parsed data to update the visual
   * data
   * Variable changed: if sound file changed -> restart from step 1. if view size changed ->
   * restart from step 2.
   *
   * @param song
   */
  ...
  private void parseVisualData() {
    ...
    mNumberPensAppearInScreen = (int) (
       ((mSeekBar.mWidth + mSeekBar.mPenDistance) / (0.0f + mSeekBar.mOneDp) + 1.0f) / 4.0f);
    ...
    for (int i_pen = 0; i_pen < mTotalPens; i_pen++) {
      mSmoothedPenGain[i_pen] = getHeight(i_pen,
         mTotalPens,
         originalPenGain,
         scaleFactor,
         minGain,
