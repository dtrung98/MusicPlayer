...
public class OptionBottomSheet extends BottomSheetDialogFragment implements View.OnClickListener {
  ...
  public View initLayout(Context context, final int[] options) {
    ...
    LinearLayout.LayoutParams imageParams = new LinearLayout.LayoutParams((int) dp * 35,
       (int) dp * 5);
    ...
    LinearLayout.LayoutParams textViewParam =
    ...
    for (int i = 0, optionsLength = options.length; i < optionsLength; i++) {
      if (options[i] == R.string.divider) {
        ...
        LinearLayout.LayoutParams divParam =
           new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
           (int) dp);
      ...
      view.setPadding((int) (12 * dp), 0, (int) (12 * dp), 0);
           ...
           /* if(options[i]==R.string.go_to_artist &&mObject instanceof Song && !((Song)mObject)
           .artistName.isEmpty()) {
  ...
  public void onClick(View view) {
    if (view instanceof TextView) {
      ...
      if (item != -1 && getActivity() != null) {
        MenuHelper.handleMenuClick((AppCompatActivity) getActivity(),
           mObject,
           mOptionStringID[item]);
        this.dismiss();
      }
    }
  }
