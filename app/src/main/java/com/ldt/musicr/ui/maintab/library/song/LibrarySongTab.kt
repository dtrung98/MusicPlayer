...
class LibrarySongTab : Fragment(R.layout.screen_songs_tab), SortOrderChangedListener,
  FirstItemCallBack, ActionResponder {
  ...
  private fun refresh() {
    refreshView.animate().rotationBy(360f)
      .setInterpolator(InterpolatorUtil.getInterpolator(6)).duration = 650
    ...
    runOnUiThread {
      if (isAdded && !isRemoving) {
        val shouldShowPreview =
          AppConfig.isShowShuffleTileInLibrarySongTab && data.isNotEmpty() && data[0] is DataItem.SongItem
