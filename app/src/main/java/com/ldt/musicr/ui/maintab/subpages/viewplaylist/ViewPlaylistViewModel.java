  ...
  public void refreshData() {
    AppExecutors.getInstance().diskIO().execute(() -> {
      ...
      if (state == null || state.mPlaylist == null) {
        reliable = Reliable.failed(MESSAGE_CODE_INVALID_PARAMS,
           new IllegalArgumentException("Require playlist parameter"));
      }

      int order = 0;
      if (reliable == null) {
        if (Util.equals(ACTION_SET_PARAMS, event.getAction()) && Util.equals(state.mPlaylist.name,
           App.getInstance()
              .getApplicationContext()
              .getResources()
              .getString(R.string.playlist_last_added))) {
          order = 2;
        } else {
          order = App.getInstance()
             .getPreferencesUtility()
             .getSharePreferences()
             .getInt("sort_order_playlist_" + state.mPlaylist.name + "_" + state.mPlaylist.id, 0);
        }
      }

      List<Song> songs = null;
      if (reliable == null) {
        songs = MusicUtil.getPlaylistSongList(App.getInstance().getApplicationContext(),
           state.mPlaylist,
           SortOrderBottomSheet.mSortOrderCodes[order]);
        if (songs == null) {
          reliable = Reliable.failed(MESSAGE_CODE_INVALID_RESPONSE,
             new NullPointerException("Null song playlist"));
        }
      }
      ...
      if (reliable == null) {
        ...
        if (latestState != null) {
          ...
          mStateLiveData.postValue(new ReliableEvent<>(Reliable.success(latestState),
             ACTION_RELOAD_DATA));
        }
      }
      ...
      if (reliable == null) {
        try {
          coverBitmap = PlaylistArtworkGenerator.getBitmap(App.getInstance()
             .getApplicationContext(), songs, false, false);
          if (coverBitmap == null) {
            reliable = Reliable.failed(MESSAGE_CODE_INVALID_RESPONSE,
               new NullPointerException("Cover bitmap is null"));
          }
        } catch (Exception e) {
      ...
      final State latestState = latestEvent != null ? latestEvent.getReliable().getData() : null;
      if (reliable != null) {
        Reliable<State> failedReliable = Reliable.custom(Reliable.Type.FAILED,
           latestState,
           reliable.mMessageCode,
           reliable.getMessage(),
           reliable.mThrowable);
