...
public class NowPlayingLayerFragment extends CardLayerFragment implements MusicServiceEventListener,
   AudioVisualSeekBar.OnSeekBarChangeListener, PalettePickerAdapter.OnColorChangedListener {
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    ViewCompat.setOnApplyWindowInsetsListener(mSpacingInsetBottom,
    ...
    mRecyclerView.setOnTouchListener((v, event) -> mCardLayerController.dispatchOnTouchEvent(mRoot,
       event));
  ...
  public void onLayerUpdate(
     ArrayList<CardLayerController.CardLayerAttribute> attrs,
    ...
    if (isAdded() && !isRemoving() && !isDetached()) {
      ...
      if (me == 1) {
        ...
        float darken =
           min + hieu * heSo_truoc + hieu * (heSo_sau - heSo_truoc) * translationPercent;
  ...
  private void handleLayerHeightChanged() {
    if (isAdded() && !isRemoving() && !isDetached()) {
      CardLayerController.CardLayerAttribute attribute =
         NowPlayingLayerFragment.this.getCardLayerController()
         .getMyAttr(NowPlayingLayerFragment.this);
  ...
  public int getLayerMinHeight(Context context, int h) {
    ...
    return systemInsetsBottom +
       (int) (context.getResources().getDimension(R.dimen.bottom_navigation_height) +
          context.getResources().getDimension(R.dimen.now_laying_height_in_minimize_mode));
  }
...
/*
  ...
  private void updatePlayingSongInfo() {
    ...
    if (song == null || song.id == -1) {
      ArrayList<Song> list = SongLoader.getAllSongs(mPlayPauseButton.getContext(),
         SortOrder.SongSortOrder.SONG_DATE);
      if (list.isEmpty()) return;
      MusicPlayerRemote.openQueue(list, 0, false);
      return;
    }
    mTitle.setText(String.format("%s %s %s",
       song.title,
       getString(R.string.middle_dot),
       song.artistName));
    ...
    if (duration > 0 && path != null && !path.isEmpty() &&
       mVisualSeekBar.getCurrentSongID() != song.id) {
      Log.d(TAG,
         "start visualize " + path + "dur = " + duration + ", pos = " +
            MusicPlayerRemote.getSongProgressMillis());
