...
public class BlurImageViewChildConstraintLayout extends ConstraintLayout {
  ...
  private void init(AttributeSet attrs) {
    ...
    TypedArray t = getContext().obtainStyledAttributes(attrs,
       R.styleable.BlurImageViewChildConstraintLayout);


    mImageViewId = t.getResourceId(R.styleable.BlurImageViewChildConstraintLayout_imageViewId,
       R.id.art);

    mBlurDelta[0] =
       (int) (t.getDimension(R.styleable.BlurImageViewChildConstraintLayout_shadowDeltaLeft,
       0));
    mBlurDelta[1] =
       (int) (t.getDimension(R.styleable.BlurImageViewChildConstraintLayout_shadowDeltaTop,
       0));
    mBlurDelta[2] =
       (int) (t.getDimension(R.styleable.BlurImageViewChildConstraintLayout_shadowDeltaRight,
       0));
    mBlurDelta[3] =
       (int) (t.getDimension(R.styleable.BlurImageViewChildConstraintLayout_shadowDeltaBottom,
       0));
    t.recycle();
  }
  ...
  private void onDrawShadow(Canvas canvas) {
    ...
    if (!shadow_drawn) {
      ...
      Log.d(TAG,
         "imageRect = [" + imageRect[0] + ", " + imageRect[1] + ", " + imageRect[2] + ", " +
            imageRect[3] + "]");
      shadow_drawn = true;
      ...
      tempCanvas.drawBitmap(bitmap,
         ...
         new Rect(imageRect[0] + mBlurDelta[0],
            imageRect[1] + mBlurDelta[1],
            imageRect[2] + mBlurDelta[2],
            imageRect[3] + mBlurDelta[3]),
         solidPaint);
      //    solidPaint.setColor(Tool.getBaseColor());
      //     solidPaint.setAlpha(80);
      //   tempCanvas.drawRect(imageRect[0]+mBlurDelta[0],imageRect[1]+mBlurDelta[1],
      //   imageRect[2]+mBlurDelta[2],imageRect[3]+mBlurDelta[3],solidPaint);
      shadowBitmap = BitmapEditor.getBlurredWithGoodPerformance(getContext(),
         tempBitmap,
         1,
         16,
         3.5f);
