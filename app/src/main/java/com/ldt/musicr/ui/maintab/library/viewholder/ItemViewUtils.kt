...
object ItemViewUtils {
  ...
  fun handleMenuClickOnNormalSongItem(
  ...
  private fun notifyPlayingStateForLegacyAdapter(
    startPosition: Int,
    viewHolder: RecyclerView.ViewHolder
  ) {
    postDelayedOnUiThread(50) {
      ...
      (viewHolder.bindingAdapter as? AbsSongAdapter)?.apply {
        notifyItemChanged(
          getMediaHolderPosition(mMediaPlayDataItem),
          AbsMediaAdapter.PLAY_STATE_CHANGED
        )
