...
public class CircularPlayPauseProgressBar extends View {
  ...
  private void init(AttributeSet attrs) {
    if (attrs != null) {
      TypedArray t = getContext().obtainStyledAttributes(attrs,
         R.styleable.CircularPlayPauseProgressBar);
      mColor = t.getColor(R.styleable.CircularPlayPauseProgressBar_iconTint,
         getResources().getColor(R.color.FlatWhite));
      t.recycle();
    }
  ...
  protected void onDraw(Canvas canvas) {
    ...
    int radius =
       Math.min(mRightWidthPadding - mLeftWithPadding, mBottomWidthPadding - mTopWithPadding) / 2;
    ...
    if (mMode == RESET) {
      if (mWaveDrawable != null) {
        ...
        mWaveDrawable.setBounds(centerX - iconRadius,
           centerY - iconRadius,
           centerX + iconRadius,
           centerY + iconRadius);
      ...
      if (mPauseDrawable != null) {
        mPauseDrawable.setBounds(centerX - iconRadius,
           centerY - iconRadius,
           centerX + iconRadius,
           centerY + iconRadius);
