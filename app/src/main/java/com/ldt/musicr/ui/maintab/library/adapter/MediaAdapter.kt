 ...
 */
open class MediaAdapter : AbsListAdapter<DataItem, RecyclerView.ViewHolder>(), SongPreviewListener,
  ActionResponder {
  ...
  override fun onSongPreviewStart(song: PreviewSong?) {
    song ?: return
    val newPosition =
      currentList.indexOfFirst { (it as? DataItem.SongItem)?.song?.id == song.song.id }
    if (newPosition > -1) {
      previewingSong = song
  ...
  fun onPlayingStateChanged() {
    ...
    AppExecutors.io().execute {
      ...
      if (playingState != playingStateNew || playingId != playingIdNew) {
        ...
        playingAdapterPosition =
          currentList.indexOfFirst { it is DataItem.SongItem && it.song.id == playingIdNew }
