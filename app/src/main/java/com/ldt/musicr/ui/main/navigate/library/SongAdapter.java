...
public class SongAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements FastScrollRecyclerView.SectionedAdapter,
            ...
            sheet.setListener(id -> {
                switch (id) {
                    ...
                    case R.id.popup_song_delete:
                        ...
                        TimberUtils.showDeleteDialog(mContext, mData.get(position).title, deleteIds, SongAdapter.this, position);
                        break;
                }
                return true;
            }
    ...
    public void shuffle() {
        ...
        handler.postDelayed(() -> {
            MusicPlayer.playAll(mContext, songIDs, mSelected, -1,TimberUtils.IdType.NA, false);
    ...
    public class ItemHolder extends RecyclerView.ViewHolder implements View.OnClickListener, View.OnAttachStateChangeListener {
        ...
        public void onClick(View view) {
            final Handler handler = new Handler();
            handler.postDelayed(() -> {
                MusicPlayer.playAll(mContext, songIDs, getRealAdapterPosition(this), -1,TimberUtils.IdType.NA, false);
                Handler handler1 = new Handler() ;
                handler1.postDelayed(() -> {
                    notifyItemChanged(currentlyPlayingPosition);
        ...
        public void bind(Song song) {
            ...
            //   ImageLoader.getInstance().displayImage(TimberUtils.getAlbumArtUri(song.albumId).toString(),mImage,new DisplayImageOptions.Builder().cacheInMemory(true).showImageOnFail(R.drawable.music_empty).resetViewBeforeLoading(true).build());
