...
class SearchFragment : FloatingViewFragment(R.layout.screen_search_floating) {
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    updateInsets()
    searchView.animate().alpha(1f).translationY(0f).setDuration(250).interpolator =
  ...
  private fun updateInsets() {
    ...
    recyclerView.setPadding(
      dpX20 + systemBarsInset[0],
      dpX52 + systemBarsInset[1],
      dpX20 + systemBarsInset[2],
      systemBarsInset[3]
    )
  }
      ...
      desTopHitSongs.forEach { song ->
        songs.add(
          DataItem.SongItem(
            ...
            songs.size,
            playlist.id,
            name = ViewUtils.getHighlightedText(
              song.title,
              song.spanPosList,
              ColorProvider.baseColorL45,
              ViewUtils.MEDIUM
            )
          )
        )
