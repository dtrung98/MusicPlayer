...
class BackStackController : CardLayerFragment(), OnPageChangeListener {
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    mViewPager.setOnTouchListener { _: View?, event: MotionEvent? ->
      mCardLayerController.dispatchOnTouchEvent(
        mRoot,
        event
      )
    }
  ...
  override fun onLayerUpdate(
    attrs: ArrayList<CardLayerAttribute>,
    actives: ArrayList<Int>,
    me: Int
  ) {
    ...
    if (me == 1) {
      ...
      var darken =
        min + distance * preFactor + distance * (postFactor - preFactor) * attrs[actives[0]].runtimePercent
  ...
  private fun doTranslateNavigation(
    attrs: ArrayList<CardLayerAttribute>,
    actives: ArrayList<Int>,
    me: Int
  ) {
  ...
  override fun onBackPressed(): Boolean {
    ...
    } else if (mViewPager.currentItem != 0) mViewPager.setCurrentItem(
      0,
      true
    ) else return mViewPager.currentItem != 0 || mNavigationAdapter!!.onBackPressed(0)
    return true
  }
  ...
  override fun onPageSelected(i: Int) {
    ...
    if (bottomNavigationView != null) {
      if (prevMenuItem != null) prevMenuItem!!.isChecked =
        false else bottomNavigationView!!.menu.getItem(0).isChecked = false
  ...
  private fun updateBackImage() {
    AppExecutors.io().execute {
      val isUseArtistImageAsBackground =
        App.getInstance().preferencesUtility.isUsingArtistImageAsBackground
      val currentSong =
        if (isUseArtistImageAsBackground) MusicPlayerRemote.getCurrentSong() else null
      val artist = currentSong?.let { MediaManager.getArtist(it.artistId) }
      postOnUiThread {
        if (isAdded && !isRemoving) {
          backgroundImageView.visibility =
            if (isUseArtistImageAsBackground) View.VISIBLE else View.GONE
