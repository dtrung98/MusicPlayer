...
class PickerRenderer(val glView: View) : GLTextureView.Renderer {
  ...
  private fun addAllItems() {
    if (adapter != null) {
      ...
      if (count != 0) {
        ...
        renderCircles.forEachIndexed { index, circleRenderItem ->
          PhysicsEngine.addCircle(
            circleRenderItem.circleBody
          )
        }
      }
    }
  ...
  override fun onSurfaceChanged(gl: GL10?, width: Int, height: Int) {
    ...
    PhysicsEngine.onViewPortSizeChanged(
      this.width,
      this.height,
      adapter?.getCircleRadiusUnit(this.width, this.height) ?: 0.125f
    )
    recreateTextures()
  }
  ...
  override fun onDrawFrame(gl: GL10?) {
    ...
    Log.d(
      "PickerRenderer",
      "current size = " + renderCircles.size + ", textureIDs size = " + textureIds?.size
    )
  ...
  fun swipe(x: Float, y: Float) = PhysicsEngine.swipe(
  ...
  fun onTap(x: Float, y: Float) = getItemPos(Vec2(x, glView.height - y)).apply {
    if (this >= 0) {
      ...
      if (PhysicsEngine.onTap(item.circleBody)) {
        listener?.let {
          if (item.circleBody.isEnhanced()) it.onBubbleDeselected(
            item.pickerItem,
            this
          ) else it.onBubbleSelected(item.pickerItem, this)
        }
      }
    }
  }
