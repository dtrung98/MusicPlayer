...
public class WavFileDescriptor {
  ...
  private AssetFileDescriptor file;                        // File that will be read from or
  // written to
  ...
  private boolean wordAlignAdjust;        // Specify if an extra byte at the end of the data
  // Wav Header
  private int numChannels;                // 2 bytes unsigned, 0x0001 (1) to 0xFFFF (65,535)
  // chunk is required for word alignment
  private long sampleRate;                // 4 bytes unsigned, 0x00000001 (1) to 0xFFFFFFFF (4,
  // 294,967,295)
  ...
  public static WavFileDescriptor openWavFile(AssetFileDescriptor file)
    ...
    if (file.getLength() != chunkSize + 8) {
      throw new WavFileException(
         "Header chunk size (" + chunkSize + ") does not match file size (" + file.getLength() +
            ")");
    }
    ...
    while (true) {
      ...
      if (chunkID == FMT_CHUNK_ID) {
        ...
        if (wavFile.validBits > 64) {
          throw new WavFileException(
             "Valid Bits specified in header is greater than 64, this is greater than a long can " +
                "hold");
        }
  ...
  public int readFrames(float[] sampleBuffer, int numFramesToRead)
     throws IOException, WavFileException {
    return readFramesInternal(sampleBuffer, 0, numFramesToRead);
  }

  private int readFramesInternal(float[] sampleBuffer, int offset, int numFramesToRead)
     throws IOException, WavFileException {
  ...
  private int readFramesInternal(int[] sampleBuffer, int offset, int numFramesToRead)
     throws IOException, WavFileException {
    if (ioState != IOState.READING) throw new IOException("Cannot read from WavFile instance");
