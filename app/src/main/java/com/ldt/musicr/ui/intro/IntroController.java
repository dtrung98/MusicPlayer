...
/**
 * Controller này tự động loại bỏ giao diện màn hình chính và hiển thị các màn hình
 * (PermissionRequire, NoSongInLibrary) khi cần
 */
public class IntroController {
  ...
  public void presentFragment(NavigationFragment fragment) {
    ...
    if (isNavigationControllerInit()) {
      ...
      //            Random r = new Random();
      //            mNavigationController.setPresentStyle(r.nextInt(39)+1); //exclude NONE
      //            present style
  ...
  private void initBackStack(AppCompatActivity activity, Bundle savedInstanceState) {
    ...
    mNavigationController = NavigationControllerFragment.navigationController(fm,
       R.id.back_wall_container);
