...
public class DarkenAndRoundedBackgroundContraintLayout extends ConstraintLayout implements
   RoundColorable {
  ...
  public DarkenAndRoundedBackgroundContraintLayout(
  ...
  private void init(AttributeSet attrs) {
    TypedArray t = getContext().obtainStyledAttributes(attrs,
       R.styleable.DarkenAndRoundedBackgroundConstraintLayout);

    color_background =
       t.getColor(R.styleable.DarkenAndRoundedBackgroundConstraintLayout_round_back_color,
       Color.WHITE);
    round_type = (t.getInt(R.styleable.DarkenAndRoundedBackgroundConstraintLayout_round_type, 0) ==
       1) ? ROUND_TYPE.ROUND_ALL : ROUND_TYPE.ROUND_TOP;
  ...
  private void drawContent(Canvas canvas, Paint paint) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && false) {
      ...
      canvas.drawPath(BitmapEditor.RoundedRect(0,
         ...
         maxRx * eachDP * number,
         maxRy * eachDP * number,
         round_type == ROUND_TYPE.ROUND_TOP), paint);
    }
  }
