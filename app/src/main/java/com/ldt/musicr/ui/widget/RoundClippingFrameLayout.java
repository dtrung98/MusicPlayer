...
public class RoundClippingFrameLayout extends SupportDarkenFrameLayout implements RoundColorable {
  ...
  protected void onDraw(Canvas canvas) {
    ...
    // canvas.drawPath(BitmapEditor.RoundedRect(0,0,getWidth(),getHeight(),maxRx*eachDP*number,
    // maxRy*eachDP*number,false),paint);
  }
  ...
  protected void dispatchSupportRoundedDraw(Canvas canvas) {
    ...
    // Bitmap bitmap1 =  Bitmap.createBitmap(canvas.getWidth(),canvas.getHeight(),Bitmap.Config
    // .ARGB_8888);
    // c.setBitmap(bitmap1);

    //        c.drawPath(BitmapEditor.RoundedRect(-getWidth()/32,-getWidth()/32,getWidth()
    //        +getWidth()/32,getHeight()+getWidth()/32,getWidth()/16,getWidth()/16,false),p);

    float value = maxRx * eachDP * number;
    p.setStrokeWidth(value);
    c.drawPath(BitmapEditor.RoundedRect(-value / 2,
       -value / 2,
       getWidth() + value / 2,
       getHeight() + value / 2,
       value,
    ...
    //        canvas.drawPath(BitmapEditor.RoundedRect(0,0,canvas.getWidth(),canvas.getHeight(),
    //        maxRx*eachDP*number,maxRy*eachDP*number,true),paint);
