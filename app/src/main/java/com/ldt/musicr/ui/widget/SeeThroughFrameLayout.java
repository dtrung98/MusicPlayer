...
public class SeeThroughFrameLayout extends FrameLayout {
  ...
  private void drawBackground() {
    ...
    //mBackgroundCanvas.drawRect(getRectSupportPadding(padding,0,0,-1,padding[3]),
    // backgroundCloudEffectPaint);
    mBackgroundCanvas.drawCircle(mBackgroundCanvas.getWidth() / 2,
       mBackgroundCanvas.getHeight() / 2,
       mBackgroundCanvas.getWidth() / 4,
       mPaint);
    //   if(applyDrawableView!=null)
    //    applyDrawableView.setImageDrawable(mBackgroundDrawable.mutate().getConstantState()
    //    .newDrawable());
  }
  ...
  private Rect getRectSupportPadding(int[] padding, int left, int top, int width, int height) {
    ...
    int Left = padding[0] + left, Top = padding[1] + top, Right = Left + width, Bottom =
       Top + height;
