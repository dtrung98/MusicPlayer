...
public class FeatureTabFragment extends MusicServiceNavigationFragment implements
   FeaturePlaylistAdapter.PlaylistClickListener, MusicServiceEventListener {
  ...
  void showSearchScreen() {
    new SearchFragment().show(getParentFragmentManager(), "search");
        /*if (getActivity() instanceof AppActivity) {
            CardLayerController.CardLayerAttribute attribute = ((AppActivity) getActivity())
            .getCardLayerController().addCardLayerFragment(ViewPlaylistCardLayerFragment
            .newInstance(PlaylistLoader.getAllPlaylistsWithAuto(requireContext()).get(0), null), 0);
  ...
  private void updateInsets() {
    ...
    mNestedScrollView.setPadding(insets[0],
       0,
       insets[2],
       (int) (insets[3] +
          mNestedScrollView.getResources().getDimension(R.dimen.bottom_back_stack_spacing)));
  ...
  public void onClickPlaylist(
    ...
    if (!showInCardLayer) {
      ...
      CardLayerController.CardLayerAttribute attribute =
         ((AppActivity) getActivity()).getCardLayerController()
         .addCardLayerFragment(sf, 0);
