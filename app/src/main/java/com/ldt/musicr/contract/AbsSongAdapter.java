...
public abstract class AbsSongAdapter extends AbsMediaAdapter<AbsBindAbleHolder, Song> implements
   SongPreviewListener {
  ...
  public void init(Context context) {
    ...
    if (context instanceof MusicServiceActivity) {
      SongPreviewController controller =
         ((MusicServiceActivity) context).getSongPreviewController();
      if (controller != null) {
  ...
  public class SongHolder extends AbsMediaHolder<Song> {
    ...
    private void bindMediaPlayState() {
      if (MusicPlayerRemote.getCurrentSong().id ==
         getData().get(getDataPosition(getAdapterPosition())).id) {
        ...
        mDescription.setTextColor(Color.argb(0xAA,
           Color.red(baseColor),
        ...
        mDescription.setTextColor(Color.argb(0xAA,
           Color.red(flatWhite),
           Color.green(flatWhite),
           Color.blue(flatWhite)));
      }
    }
    ...
    void clickPresent() {
      ...
      if (mPreviewSong != null &&
         mPreviewSong.getSong().equals(getData().get(getDataPosition(getAdapterPosition())))) {
        resetProgress();
        forceStopPreview();
      } else {
