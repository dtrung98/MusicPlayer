...
object SearchUtils {
   ...
   */
  @JvmStatic
  fun <T> filterTopHitEntities(
    srcList: List<T>,
    desList: MutableList<T>,
    keyword: String,
    @SearchConstant.MatchFlag flags: Int = 0
  ) where T : Entity {
    srcList.forEach {
      val matchScore =
        retrieveMatchScore(keyword, it.displayName, null, null, it, it.spanPosList, flags)
  ...
  private fun retrieveMatchScore(
    ...
    val arrayItemWordOrg: Array<String> = when {
      paramArrayItemWord != null -> {
        ...
        if (hasFlags(flags, SearchConstant.SEARCH_FLAG_TO_LOW_CASE)) {
          ...
          arrayItemWordLowercase =
            Array(paramArrayItemWord.size) { paramArrayItemWord[it].lowercase() }
      ...
      search != null -> {
        ...
        arrayItemWordLowercase =
          ...
          ) searchOutputOrg.arrayWords else SearchPreProcessingProvider.get(
            search,
    ...
    val arrayKeyWord: Array<String> = when {
      ...
      key != null -> {
        ...
        val arrayKeyWord = if (hasFlags(
            flags,
            SearchConstant.SEARCH_FLAG_SEARCH_NBS
          )
        ) SearchPreProcessingProvider.get(
          key,
        ...
        arrayKeyWordKeepAccents = if (hasFlags(
            flags,
            SearchConstant.SEARCH_FLAG_SEARCH_NBS
          )
        ) SearchPreProcessingProvider.get(
    ...
    var numberFullMatchKeepAccents =
      0 // number full match in accent words between search words array and key words array
    for (i in arrayKeyWord.indices)  //for each work in key word string - example: One Two Three
    {
      ...
      for (t in 0 until sizeDpnElement)  // for each work in Display name string - example: Peter One.
      {
        ...
        if (t == 0) preCalStartPos =
          0 else preCalStartPos += (arrayItemWordLowercase[t - 1].length + 1) //plus 1 for space
        ...
        while (index >= 0 && countLoopToFind <= 10) {
          if (index >= 0) {
            ...
            if (hasFlags(flags, SearchConstant.SEARCH_FLAG_ALLOW_MATCH_CASE_CONTAIN)) {
              ...
              match =
                (Character.isUpperCase(c1) && (Character.isLowerCase(c2) || (c2 > 'z') || (c2 < 'A')))
            }
            if (match) {
              try {
                if (arrListPostStartEndMatch[t] != null && arrListPostStartEndMatch[t]!!.size > 0) {
                  ...
                  while (l < arrListPostStartEndMatch[t]!!.size - 1) {
                    if (index >= arrListPostStartEndMatch[t]!![l] && index < arrListPostStartEndMatch[t]!![l + 1]) {
                      needToBreak = true
                      countLoopToFind++
                      index = arrayItemWordLowercase[t].indexOf(
                        (arrayKeyWord[i]),
                        index + (arrayKeyWord[i].length) /*dpnAlreadyMatch[t]*/
                      )
                      break
                    }
                    l += 2
      ...
      try {
        if (bestPos >= 0) {
          val percent =
            (arrayKeyWord[i].length.toFloat() / arrayItemWordLowercase[bestPos].length) //for compare percent special word
          var indexStartMatch = 0
          var indexEndMatch = 0
          if (arrayKeyWord[i].length != arrayItemWordLowercase[bestPos].length) {
            indexStartMatch =
              arrayItemWordLowercase[bestPos].indexOf((arrayKeyWord[i]), startMatchPos)
            ...
            } else arrListPostStartEndMatch[bestPos] =
              ArrayList(Arrays.asList(indexStartMatch, indexEndMatch))
          }
          if (percent == 1f) {
          ...
          if (arrayItemWordOffsets == null) {
            ...
            startPos = lowerDpn.indexOf(
              (arrayKeyWord[i]),
              if (currentPosToFind >= 0) (currentPosToFind + startMatchPos) else 0
            )
          ...
          if (arrayKeyWord[i].length == arrayItemWordLowercase[bestPos].length) dpnAlreadyMatch[bestPos] =
            -1
          if (startPos != -1) {
...
//                    }
        ...
        val hasContainAccentsAndFullMatchKeepAccents =
          fullMatchAccentsBonusChecking && hasContainAccentsInKey && (numberFullMatchKeepAccents == arrayKeyWord.size)
  ...
  private fun filterTopHitSample() {
    ...
    srcList.forEach {
      val matchScore = retrieveMatchScore(
        null,
        null,
        emptyArray(),
        emptyArray(),
