...
object SearchPreProcessingProvider {
  ...
  private fun create(
    ...
    return when {
      flags.hasFlags(SearchConstant.SEARCH_FLAG_PREPROCESS_TYPE_KEY) -> SearchPreProcessingUtils.preprocessTypeKey(
        input,
        flags
      )
      flags.hasFlags(SearchConstant.SEARCH_FLAG_PREPROCESS_TYPE_KEY_NBS) -> SearchPreProcessingUtils.preprocessTypeKeyUsername(
        input,
        flags
      )
      else /*SearchConstant.hasFlags(flags, SearchConstant.SEARCH_FLAG_PREPROCESS_TYPE_SEARCH)*/ -> SearchPreProcessingUtils.preprocessTypeSearch(
        input,
        flags
      )
    }
...
open class PreProcessingOffsetOutput(arrayWords: Array<String>, val arrayWordOffsets: IntArray) :
